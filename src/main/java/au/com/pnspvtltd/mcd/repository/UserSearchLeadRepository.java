...
public interface UserSearchLeadRepository extends JpaRepository<Search, Long>{
	...
	@Query("SELECT search FROM Search search WHERE search.modelDisplay=?1" +
			" ORDER BY search.carSearchId")
			public List<Search> getSearchModel(String modelDisplay);
	
	@Query("SELECT search FROM Search search WHERE search.modelName=?1" +
			" ORDER BY search.carSearchId")
			public List<Search> getSearchDisp(String modelName);
	
	@Query("SELECT search FROM Search search WHERE search.modelTrim=?1" +
			" ORDER BY search.carSearchId")
			public List<Search> getSearchTrim(String modelTrim);
	
	@Query("SELECT search FROM Search search WHERE search.creationDate=?1" +
			" ORDER BY search.carSearchId")
