...
var ServiceLevelCarQuery = function(){}

ServiceLevelCarQuery.prototype = {
 ...
 //Set the change event for the models dropdown to save the selected model
    ...
    initYearMakeModelTrim12: function(year_select_id, make_select_id, model_select_id)
    {
    //alert("came here12");
    ...
    initFinanceProviders: function(finance_id)
    {
     ...
     this.populateFinanceProv(finance_id);

     var sender = this;

     
    },
    populateFinanceProv: function(finance_id)
    {
     //Set a loading message while we retrieve the data
     $("select#"+this.finance_id).html("<option value=''>Loading Finance Providers...</option>");
    ...
    }, // end Finance Providers
    
    
// start of insurance providers
    initInsuranceProviders: function(insurance_id)
    {
        //alert("came here");
    	//alert("came here without 12");
    	//Set the ids for the select elements
     ...
     this.populateInsuranceProv(insurance_id);

     var sender = this;

     
    },
    populateInsuranceProv: function(insurance_id)
    {
     //Set a loading message while we retrieve the data
     $("select#"+this.insurance_id).html("<option value=''>Loading Insurance Providers...</option>");

        var sender = this;
        //this.base_url = 'http://localhost:8080/MyCarDomain/api/tempCarModelYears';
        this.base_url = 'api/insuranceProv';
        //alert(base_url);
        //alert(this.base_url);
        $.getJSON(this.base_url, function(data) {
    ...
    initYearMakeModelTrim: function(year_select_id, make_select_id)
    {
        //alert("came here");
    	//alert("came here without 12");
    	//Set the ids for the select elements
 ...
 //Set a loading message while we retrieve the data
        $("#"+this.make_list_id).html("<fieldset><p>Makes:</p><div class='scrollable'><p>Loading States...</p></div></fieldset>");
...
//alert("out"+out);
 ...
 if(this.cur_year == "")
 {
  $("select#"+this.make_select_id).html(this.empty_option_html);
      ...
      //Set a loading message while we retrieve the data
        $("select#"+this.make_select_id).html("<option value=''>Loading level2...</option>");
        ...
        this.base_url = 'api/level2';
        //alert(this.base_url);
        //Get Car Model JSON for the selected make
     $.getJSON(this.base_url, {level1:this.cur_year}, function(data) {
...
//  $("select#"+this.model_select_id).html(this.empty_option_html);
  //$("select#"+this.trim_select_id).html(this.empty_option_html);
      ...
      //Set a loading message while we retrieve the data
        $("select#"+this.make_select_id).html("<option value=''>Loading Level 2...</option>");
...
//      $("select#"+this.model_select_id).html(this.empty_option_html);
  // $("select#"+this.trim_select_id).html(this.empty_option_html);
     ...
     //Get Car Model JSON for the selected make
 $.getJSON(this.base_url, {country:this.cur_year, state:this.cur_make}, function(data) {
  ...
  if(!sender.responseError(data))
  {
    ...
    modelSelectChange: function ()
    {
        ...
        this.base_url =	'api/postcodes';
        //Get Car Model JSON for the selected make
     $.getJSON(this.base_url, {country:this.cur_year, state:this.cur_make, region:this.cur_model}, function(data) {
